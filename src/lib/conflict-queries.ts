...
export async function pruneStaleTbaConflicts(): Promise<number> {
  const prisma = getPrisma();
  const rows = await prisma.eventConflict.findMany({
    where: { isAutoGenerated: true, resolutionStatus: "unresolved" },
    include: { eventA: true, eventB: true },
  });

  const staleIds = rows
    .map((row) => {
      const conflict = toConflictRecord(row);
      return conflict && isStaleTbaConflict(conflict) ? row.id : null;
    })
    .filter((id): id is string => id !== null);

  if (staleIds.length === 0) return 0;

  const result = await prisma.eventConflict.deleteMany({
...
export async function recalculateConflicts(): Promise<{
  ...
  for (let i = 0; i < rawEvents.length; i += BATCH_SIZE) {
    ...
    await Promise.all(
      chunk.map(async (row) => {
        ...
        classifiedEvents.push({ ...dto, ...classification });
  ...
  const detected = detectConflicts(classifiedEvents).filter((c) => !isStaleTbaConflict(c));
